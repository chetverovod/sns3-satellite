...
#ifndef SAT_INPUT_FSTREAM_TIME_LONG_DOUBLE_CONTAINER_H
#define SAT_INPUT_FSTREAM_TIME_LONG_DOUBLE_CONTAINER_H
 ...
 * \ingroup satellite
 *
 * \brief Class for input file stream container for storing long double values.
 ...
 */
class SatInputFileStreamTimeLongDoubleContainer : public Object
{
  public:
    /**
     ...
     */
    SatInputFileStreamTimeLongDoubleContainer(std::string filename,
  ...
  private:
     ...
     */
    bool ReadNextDataRow(std::vector<long double>& row);
     ...
     * member variable. \param lastValidPosition position of last matching value \param
     * timeShiftValue value to shift the time if needed \param comparisonValue value which next
     * closest match to find \return was next time sample found
     */
    bool FindNextClosest(uint32_t lastValidPosition,
                         long double timeShiftValue,
                         double long comparisonTimeValue);
