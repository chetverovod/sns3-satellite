...
{
    static TypeId tid =
        TypeId("ns3::LoraNetworkScheduler")
            .SetParent<Object>()
            .AddAttribute("FirstWindowAnswerDelay",
                          "Delay to wait between end of reception of paquet and sending of anwser, "
                          "to be in first window opportunity",
                          TimeValue(Seconds(1)),
                          MakeTimeAccessor(&LoraNetworkScheduler::m_firstWindowAnswerDelay),
                          MakeTimeChecker())
            .AddAttribute("SecondWindowAnswerDelay",
                          "Delay to wait between end of reception of paquet and sending of anwser, "
                          "to be in second window opportunity",
                          TimeValue(Seconds(2)),
...
LoraNetworkScheduler::OnReceivedPacket(Ptr<const Packet> packet)
{
    ...
    if (!m_status->GetEndDeviceStatus(packet)->HasReceiveWindowOpportunityScheduled())
    {
        ...
        m_status->GetEndDeviceStatus(packet)->SetReceiveWindowOpportunity(
            Simulator::Schedule(m_firstWindowAnswerDelay,
    ...
    if (gwAddress == Address() && window == 1)
    {
        ...
        m_status->GetEndDeviceStatus(deviceAddress)
            ->SetReceiveWindowOpportunity(
                Simulator::Schedule(m_secondWindowAnswerDelay,
                                    &LoraNetworkScheduler::OnReceiveWindowOpportunity,
                                    this,
                                    deviceAddress,
                                    2));
